...
class AdministraController extends Controller
{
    ...
    public function primeraDiapositiva()
    {
        $projectes = Projecte::all();
        return view('admin.primeraDiapositiva.index')->with('projectes', $projectes);
    }
    public function segonaDiapositiva()
    {
        $folderPath = public_path('img/slides/slide2');
        $images = [];
        ...
        return view('admin.segonaDiapositiva.index', compact('images'));
    }
    public function terceraDiapositiva()
    {
        $folderPath = public_path('img/slides/slide3');
        $images = [];
        ...
        return view('admin.terceraDiapositiva.index', compact('images'));
    }
    public function quartaDiapositiva()
    {
        $folderPath = public_path('img/slides/slide4');
        $images = [];
        ...
        return view('admin.quartaDiapositiva.index', compact('images'));
    }
    public function cinquenaDiapositiva()
    {
        $folderPath = public_path('img/slides/slide5');
        $images = [];
        ...
        return view('admin.cinquenaDiapositiva.index', compact('images'));
    }
    public function sisenaDiapositiva()
    {
        $projectes = Projecte::all();
        return view('admin.sisenaDiapositiva.index')->with('projectes', $projectes);
    }
    public function imatgePantalla(Request $request, $id){
        try {
            $request->validate([
                'file1' => 'required|image|mimes:jpeg,png,jpg,gif,svg|max:2048',
            ]);
        } catch (\Illuminate\Validation\ValidationException $e) {
            return back()->with('error_message', 'No s\'ha carregat cap imatge o la imatge no és vàlida.');
        }
        // Comprovar si el fitxer està present i és vàlid
        if ($request->hasFile('file1') && $request->file('file1')->isValid()) {
            // Obtenir el fitxer de la petició
            $file = $request->file('file1');

            $imageName = $file->getClientOriginalName();

            // Moure el fitxer a la carpeta corresponent
            $file->move(public_path('img/slides/slide' . $id), $imageName);

            // Retornar el camí complet del fitxer mogut
            return back()->with('message', 'Imatge carregada correctament.');
        }else{
            // En cas que el fitxer no sigui vàlid o no s'hagi pujat, retornar un missatge d'error
            return back()->with('error_message', 'No s\'ha carregat cap imatge o la imatge no és vàlida.');
        }

        
    }
    public function eliminaImatgePantalla($id,$nomImatge)
    {
        $image_path = public_path('img/slides/slide'. $id . '/').$nomImatge;
            //Borro la imatge
            if (file_exists($image_path)) {
                File::delete($image_path);
            }

            return back()->with('message', 'Imatge suprimida correctament.');
    }
    /**
     * Mostra les entrades
     *
